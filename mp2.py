...
from PySide6.QtCore import QTimer, QTime, Qt, QObject, Signal
...
class VlcVideoWidget(QWidget):
    def __init__(self, parent=None):
        ...
        # 关闭硬解，规避你报的 D3D11VA / get_buffer / thumbnail 等问题
    ...
    def _on_end_reached(self, event):
        # VLC 事件线程 -> Qt 主线程
        self._bridge.end_reached.emit()

    def set_media(self, file_path: str) -> bool:
        try:
            self.bind_output()
    ...
    def set_position(self, p: float):
        try:
            self.player.set_position(float(p))
        except Exception:
            pass

    def set_volume_0_100(self, vol: int):
...
class OnDemandWindow(QMainWindow):
    ...
    def __init__(self, on_finish, get_volume_0_100, get_muted):
        ...
        # VLC 播放区
        self.video = VlcVideoWidget(self)
        self.video._bridge.end_reached.connect(self._on_end_reached)
        ...
        self.progress_slider.sliderMoved.connect(self._seek_slider)
    ...
    def apply_volume_state(self):
        ...
        self.video.set_mute(muted)
        if not muted:
            self.video.set_volume_0_100(vol)

    def set_volume_from_main(self, vol_0_100: int, muted: bool):
        self.video.set_mute(bool(muted))
        if not muted:
            self.video.set_volume_0_100(int(vol_0_100))
    ...
    def _seek_slider(self, v: int):
        if self.video.get_length_ms() <= 0:
            return
        self.video.set_position(v / 1000.0)

    def update_progress(self):
        length = self.video.get_length_ms()
        if length <= 0:
        ...
        cur_s = self.video.get_time_ms() // 1000
        tot_s = length // 1000
        self.time_label.setText(f"{time.strftime('%M:%S', time.gmtime(cur_s))} / {time.strftime('%M:%S', time.gmtime(tot_s))}")
    ...
    def play_current(self):
        self.apply_volume_state()
        p = str(self.playlist[self.index])
        ok = self.video.set_media(p)
        if not ok:
            return
        self.video.play()
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Time Scheduler Player (VLC)")
        ...
        # ===== 主 VLC 播放区 =====
        self.video = VlcVideoWidget(self)
        self.video._bridge.end_reached.connect(self.on_end_reached)
        ...
        self.apply_volume_to_main()

    # =====================
    # 音量控制（主控，联动点播窗口）
    # =====================

    def apply_volume_to_main(self):
        self.video.set_mute(self.muted)
        if not self.muted:
            self.video.set_volume_0_100(self.volume)
    ...
    def toggle_run(self):
        if not self.running:
            ...
            # 开始运行
            self.running = True
            self.run_btn.setText("结束运行")
            self.clock_timer.start(1000)

            # 关键：立即调度一次（不等 1 秒），确保“点击 run 立刻有反应”
    ...
    def play_current(self):
        ...
        self.apply_volume_to_main()
        fp = str(self.current_playlist[self.current_index])
        ok = self.video.set_media(fp)
        if not ok:
            return
        self.video.play()

    def on_end_reached(self):
        """主播放器播完一个文件的回调（来自 VLC EndReached）"""
        if not self.running or self.in_interrupt:
            return
    ...
    def start_on_demand(self):
        ...
        pos_ms = self.video.get_time_ms()
        wnext = self.waiting_next_segment
        self.interrupt_state = (seg, plist, idx, pos_ms, wnext)
    ...
    def resume_from_interrupt(self, terminated: bool):
        ...
        seg, plist, idx, pos_ms, wnext = self.interrupt_state
        self.interrupt_state = None
        ...
        if seg and active == seg and plist:
            ...
            # 让 VLC 有时间加载再跳转时间
            QTimer.singleShot(200, lambda: self.video.set_time_ms(pos_ms))
            return
